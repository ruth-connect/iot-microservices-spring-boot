...
public class MiLightWiFiBox {
	...
	/**
	 * The maximum brightness value, starting at 2.
	 */
	 ...
	 */
	public MiLightWiFiBox(InetAddress address, int port) {
		super();
	 ...
	 *            is the address of the WiFi box
	 */
	public MiLightWiFiBox(InetAddress address) {
	 ...
	 */
	public MiLightWiFiBox(String host, int port) throws UnknownHostException {
	 ...
	 *             if the hostname could not be resolved
	 */
	public MiLightWiFiBox(String host) throws UnknownHostException {
			...
			public void run() {
				try {
					for (byte[] message : messages) {
						MiLightWiFiBox.this.sendMessage(message);
						Thread.sleep(sleep);
					}
				} catch (IOException e) {
					// if the message could not be sent
